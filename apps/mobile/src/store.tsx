...
function useAppState(startTab: Tab = 'home') {
  ...
  const [plStart, setPlStart] = useState<'connect' | 'persona' | 'deposit'>('connect');   // 페르소나 플로우 시작 단계(홈=풀, 온보딩=판독부터, 가계부=배분만)
  ...
  const actions = {
    ...
    // 페르소나→배분 플로우 — 홈 미션은 연결부터, 온보딩은 판독부터(연결 완료 상태), 가계부 입금 카드는 배분만
    openAllocFlow: (start: 'connect' | 'persona' | 'deposit') => { setPlStart(start); setPush('personaLedger'); setSheet(null); },
